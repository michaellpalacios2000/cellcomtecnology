...
// ║    Se_Probo OBLIGATORIO + DNI Cliente + Nombre_Producto      ║
...
const POS = {
  ...
  // ✅ NUEVO: Se probó — OBLIGATORIO
  seProbo:          '', // 'SI' o 'NO'

  // ✅ NUEVO: Cliente por DNI (opcional)
  idClienteActual:  '',
  nombreCliente:    '',
  dniActual:        '',
...
// ════════════════════════════════════════════════════════════════
// ✅ SE PROBÓ EL PRODUCTO — OBLIGATORIO
// ════════════════════════════════════════════════════════════════

function seleccionarSeProbo(valor) {
  POS.seProbo = valor;
  if (navigator.vibrate) navigator.vibrate(15);

  const btnSI   = document.getElementById('btnSeProboSI');
  const btnNO   = document.getElementById('btnSeProboNO');
  const indic   = document.getElementById('seProboIndicador');

  // Resetear ambos
  if (btnSI) {
    btnSI.className = 'se-probo-btn';
    btnSI.style.cssText = '';
  }
  if (btnNO) {
    btnNO.className = 'se-probo-btn';
    btnNO.style.cssText = '';
  }

  if (valor === 'SI') {
    if (btnSI) btnSI.classList.add('si-activo');
    if (indic) {
      indic.style.display     = 'block';
      indic.style.background  = '#F0FFF4';
      indic.style.color       = '#276749';
      indic.style.borderColor = 'rgba(72,187,120,0.3)';
      indic.textContent       = '✅ Producto probado — Listo para vender';
    }
  } else if (valor === 'NO') {
    if (btnNO) btnNO.classList.add('no-activo');
    if (indic) {
      indic.style.display     = 'block';
      indic.style.background  = '#FFF5F5';
      indic.style.color       = '#C53030';
      indic.style.borderColor = 'rgba(252,129,129,0.3)';
      indic.textContent       = '❌ No fue probado — Se registrará así';
    }
  }
}

// ════════════════════════════════════════════════════════════════
// ✅ DNI — BÚSQUEDA DE CLIENTE (OPCIONAL)
// ════════════════════════════════════════════════════════════════
...
async function buscarClienteDNI() {
  ...
  if (!dni) {
    mostrarToast('📝 Ingresa el DNI para buscar', 'warning');
    document.getElementById('inputDNI')?.focus();
    return;
  }

  if (!/^\d{8}$/.test(dni)) {
    mostrarToast('⚠️ El DNI debe tener exactamente 8 dígitos numéricos', 'warning');
    return;
  }

  try {
    const resultado = await API.get('buscarClientePorDNI', { dni });

    if (resultado.success && resultado.data?.encontrado) {
      const cliente       = resultado.data.cliente;
      POS.idClienteActual = cliente.ID_Cliente;
...
async function registrarNuevoCliente(dni) {
  const { value: datos } = await Swal.fire({
    ...
    html: `
      <div style="font-family:'Inter',sans-serif;text-align:left;">
        <div style="background:#EBF4FF;border-radius:10px;padding:10px;margin-bottom:14px;">
          <span style="font-size:13px;color:#2E6DB4;font-weight:700;">DNI: ${dni}</span>
        </div>
        <label style="font-size:11px;font-weight:700;color:#4A5568;text-transform:uppercase;letter-spacing:0.5px;">
          Nombre completo *
        </label>
        <input id="cliNombre" class="swal2-input"
               placeholder="Nombre del cliente"
               style="font-size:14px;margin:6px 0 12px;">
        <label style="font-size:11px;font-weight:700;color:#4A5568;text-transform:uppercase;letter-spacing:0.5px;">
          Teléfono WhatsApp (opcional)
        </label>
        <input id="cliTelefono" class="swal2-input"
               type="tel" placeholder="987654321"
               inputmode="tel" style="font-size:14px;margin-top:6px;">
      </div>
    `,
    showCancelButton:  true,
    confirmButtonText: '✅ Registrar cliente',
    cancelButtonText:  'Cancelar (venta rápida)',
    ...
    preConfirm: () => {
      ...
      return {
        Nombre:      nombre,
        Telefono_WA: document.getElementById('cliTelefono').value.trim(),
        DNI:         dni
      };
    }
  });
  ...
  if (!datos) {
    limpiarCliente();
    mostrarToast('ℹ️ Continuando como venta rápida sin cliente', 'info', 2000);
    return;
  }

  try {
    const resultado = await API.post('registrarClienteConDNI', datos);
...
function limpiarFormulario() {
  ...
  // Resetear botones Se_Probo
  const btnSI = document.getElementById('btnSeProboSI');
  const btnNO = document.getElementById('btnSeProboNO');
  const indic = document.getElementById('seProboIndicador');

  if (btnSI) { btnSI.className = 'se-probo-btn'; btnSI.style.cssText = ''; }
  if (btnNO) { btnNO.className = 'se-probo-btn'; btnNO.style.cssText = ''; }
...
// ════════════════════════════════════════════════════════════════
// REGISTRAR VENTA — CON Se_Probo OBLIGATORIO
// ════════════════════════════════════════════════════════════════
...
async function registrarVenta() {
  ...
  // ✅ VALIDAR Se_Probo — OBLIGATORIO
  if (!POS.seProbo || (POS.seProbo !== 'SI' && POS.seProbo !== 'NO')) {
    mostrarToast('⚠️ Indica si el producto fue probado — ¡Es OBLIGATORIO!', 'warning');

    // Animar botones para llamar la atención
    const btnSI = document.getElementById('btnSeProboSI');
    const btnNO = document.getElementById('btnSeProboNO');
    ...
    // Scroll suave hasta los botones
    ...
    const confirma = await Swal.fire({
      ...
      html:              `
        <div style="font-family:'Inter',sans-serif;">
          <p style="color:#718096;">La ganancia es negativa:</p>
          <div style="font-size:24px;font-weight:900;color:#C53030;margin:10px 0;">
            -S/${Math.abs(ganancia).toFixed(2)}
          </div>
          <p style="font-size:13px;color:#718096;">¿Deseas continuar de todas formas?</p>
        </div>`,
      icon:              'warning',
      showCancelButton:  true,
  ...
  try {
    ...
    if (resultado.success) {
      ...
      const msgCliente = POS.nombreCliente ? ` · 👤 ${POS.nombreCliente.split(' ')[0]}` : '';
      mostrarToast(
        `✅ S/${subtotal.toFixed(2)} — ${POS.metodoPago}${msgCliente}`,
        'success'
      );
...
async function agregarAlInventario() {
  ...
  try {
    const { value: categoria } = await Swal.fire({
      ...
      html: `
        <div style="font-family:'Inter',sans-serif;text-align:left;">
          <div style="background:#EBF4FF;border-radius:10px;padding:12px;margin-bottom:12px;">
            <div style="font-size:14px;font-weight:700;">${producto}</div>
            <div style="font-size:12px;color:#4A5568;margin-top:4px;">
              Precio: S/${precio.toFixed(2)} | Costo: S/${costo.toFixed(2)} | Stock: ${cantidad}
            </div>
          </div>
          <label style="font-size:11px;font-weight:700;color:#4A5568;text-transform:uppercase;">
            Categoría *
          </label>
        </div>`,
      input: 'select',
  ...
  try {
    ...
    if (resultado.success && resultado.data) {
      ...
      POS.ventasHoy = ultimasVentas.map(v => ({
        ...
        // ✅ Usa Nombre_Producto si existe (col K), sino ID_Producto
        producto: v.items?.[0]?.Nombre          ||
                  v.items?.[0]?.Nombre_Producto  ||
                  v.items?.[0]?.ID_Producto       || 'Venta',
    ...
    <div class="venta-item"
      ...
      <div class="venta-desc">
        ...
        <div class="venta-detalle">
          ...
          ${v.nota ? v.nota + ' · ' : ''}${v.metodo}
          ${v.seProbo === 'SI' ? ' · ✅' : v.seProbo === 'NO' ? ' · ❌' : ''}
          ${v.cliente ? ` · 👤 ${v.cliente.split(' ')[0]}` : ''}
        </div>
      </div>
    ...
    html: `
      <div style="text-align:left;font-family:'Inter',sans-serif;">
        <div style="margin-bottom:12px;">
          <div style="font-size:11px;color:#718096;text-transform:uppercase;font-weight:600;margin-bottom:4px;">Producto</div>
        ...
        <div style="background:#F7FAFC;border-radius:10px;padding:10px;font-size:12px;color:#718096;line-height:1.7;">
          🕐 ${v.hora} &nbsp;·&nbsp; 💳 ${v.metodo} &nbsp;·&nbsp; 📦 ${v.cantidad} und.
          ${v.costo > 0 ? `<br>💸 Costo: S/${(v.costo / v.cantidad).toFixed(2)} c/u` : ''}
          ${v.seProbo ? `<br>${v.seProbo === 'SI' ? '✅ Fue probado' : '❌ No fue probado'}` : ''}
...
async function cerrarCaja() {
  ...
  if (Math.abs(descuadre) < 0.01) {
    iconoCierre   = '✅'; tituloCierre  = '¡Caja Cuadrada!';
    colorCierre   = '#276749'; mensajeCierre = 'El efectivo coincide exactamente. ¡Perfecto!';
    bgCierre      = '#F0FFF4'; borderCierre  = 'rgba(72,187,120,0.4)';
  } else if (descuadre > 0) {
    iconoCierre   = '🟡'; tituloCierre  = 'Hay un Sobrante en Caja';
    colorCierre   = '#C05621'; mensajeCierre = `Hay S/${descuadre.toFixed(2)} de más en caja.`;
    bgCierre      = '#FFFAF0'; borderCierre  = 'rgba(246,173,85,0.4)';
  } else {
    iconoCierre   = '🔴'; tituloCierre  = 'Hay un Faltante en Caja';
    colorCierre   = '#C53030'; mensajeCierre = `Faltan S/${Math.abs(descuadre).toFixed(2)} en caja.`;
    bgCierre      = '#FFF5F5'; borderCierre  = 'rgba(252,129,129,0.4)';
  }
      ...
      <div style="font-family:'Inter',sans-serif;text-align:left;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px;">
          <div style="background:#EBF4FF;border-radius:12px;padding:12px;text-align:center;">
            <div style="font-size:10px;color:#4A90D9;font-weight:700;text-transform:uppercase;">Esperado</div>
          ...
          <div style="background:${bgCierre};border:1.5px solid ${borderCierre};border-radius:12px;padding:12px;text-align:center;">
            <div style="font-size:10px;color:${colorCierre};font-weight:700;text-transform:uppercase;">Contado</div>
        ...
        <div style="background:#F7FAFC;border-radius:12px;padding:12px;font-size:12px;color:#4A5568;line-height:1.8;">
          <div style="display:flex;justify-content:space-between;">
            <span>📊 Total ventas:</span><span style="font-weight:700;">${POS.ventasHoy.length}</span>
          </div>
          <div style="display:flex;justify-content:space-between;">
  ...
  try {
    ...
    if (resultado.success) {
      ...
      mostrarToast(POS.cierreData.descuadreTexto, Math.abs(descuadre) < 0.01 ? 'success' : 'warning');

      const { isConfirmed: quierePDF } = await Swal.fire({
        title:             '📄 ¿Exportar resumen del día?',
        html:              '<p style="font-family:\'Inter\',sans-serif;font-size:13px;color:#4A5568;">PDF profesional con arqueo, descuadre y detalle de ventas.</p>',
        icon:              'question',
        showCancelButton:  true,
  ...
  POS.ventasHoy.forEach(v => {
    filasVentas += `
      <tr>
        ...
        <td style="padding:8px 10px;border-bottom:1px solid #EBF4FF;font-size:12px;font-weight:600;">
          ${v.producto}
          ${v.seProbo ? `<span style="font-size:9px;color:#718096;"> (${v.seProbo === 'SI' ? '✅ prob.' : '❌ no prob.'})</span>` : ''}
        </td>
        ...
        <td style="padding:8px 10px;border-bottom:1px solid #EBF4FF;font-size:12px;">${v.cliente || '—'}</td>
      </tr>`;
  });
...
<head>
  ...
  <style>
    ...
    * { margin:0; padding:0; box-sizing:border-box; }
    body { font-family:'Inter',sans-serif; background:#fff; color:#1A1A2E; padding:28px; font-size:13px; }
    .header { display:flex; align-items:center; justify-content:space-between; margin-bottom:20px; padding-bottom:16px; border-bottom:3px solid #4A90D9; }
    ...
    .kpi  { flex:1; border-radius:12px; padding:12px; text-align:center; border:1.5px solid; }
    .kpi-v { font-size:20px; font-weight:900; }
    .kpi-l { font-size:9px; color:#718096; font-weight:700; text-transform:uppercase; letter-spacing:0.3px; margin-top:3px; }
    .kpi.a { background:#EBF4FF; border-color:#D4E9FF; } .kpi.a .kpi-v { color:#2E6DB4; }
    .kpi.g { background:#F0FFF4; border-color:#C6F6D5; } .kpi.g .kpi-v { color:#276749; }
    .kpi.r { background:#FFF5F5; border-color:#FED7D7; } .kpi.r .kpi-v { color:#C53030; }
    .kpi.p { background:#FAF5FF; border-color:#E9D8FD; } .kpi.p .kpi-v { color:#6B46C1; }
    .caja-box { background:linear-gradient(135deg,#0F2044,#2E6DB4); border-radius:14px; padding:18px; color:#fff; margin-bottom:20px; }
    .caja-g { display:flex; justify-content:space-around; text-align:center; }
    .caja-l { font-size:10px; color:rgba(255,255,255,0.55); text-transform:uppercase; }
    .caja-v { font-size:17px; font-weight:900; margin-top:4px; }
    .sec-t { font-size:13px; font-weight:800; color:#1A1A2E; margin:20px 0 10px; padding-bottom:6px; border-bottom:2px solid #EBF4FF; }
    table { width:100%; border-collapse:collapse; margin-bottom:18px; }
    thead { background:linear-gradient(135deg,#EBF4FF,#D4E9FF); }
    th { padding:9px 10px; font-size:10px; font-weight:800; color:#2E6DB4; text-transform:uppercase; letter-spacing:0.3px; text-align:left; }
    .footer { text-align:center; margin-top:20px; padding-top:14px; border-top:2px solid #EBF4FF; font-size:10px; color:#A0AEC0; line-height:1.6; }
    .no-print { text-align:center; margin-top:20px; }
    .btn-p { padding:12px 32px; background:linear-gradient(135deg,#4A90D9,#2E6DB4); color:#fff; border:none; border-radius:12px; font-size:14px; font-weight:700; cursor:pointer; font-family:'Inter',sans-serif; }
...
<body>
  <div class="header">
    <div>
      ...
      <p>Cierre y Resumen del Día — POS v1.0.0</p>
      <p>Cieneguilla, Lima, Perú</p>
    </div>
    <div style="text-align:right;font-size:12px;color:#718096;">
      <strong style="font-size:15px;color:#1A1A2E;">${fecha}</strong><br>
      Hora: ${hora}<br>
  ...
  <div class="kpis">
    <div class="kpi a"><div class="kpi-v">S/${POS.totalVentasHoy.toFixed(2)}</div><div class="kpi-l">💰 Ventas totales</div></div>
    <div class="kpi g"><div class="kpi-v">S/${POS.totalGananciaHoy.toFixed(2)}</div><div class="kpi-l">📈 Ganancia bruta</div></div>
    <div class="kpi r"><div class="kpi-v">S/${POS.totalGastosHoy.toFixed(2)}</div><div class="kpi-l">💸 Gastos</div></div>
    <div class="kpi p"><div class="kpi-v">S/${(POS.totalGananciaHoy - POS.totalGastosHoy).toFixed(2)}</div><div class="kpi-l">💎 Ganancia neta</div></div>
    <div class="kpi a"><div class="kpi-v">${POS.ventasHoy.length}</div><div class="kpi-l">🛒 Transacciones</div></div>
  </div>

  <div class="caja-box">
    <div style="font-size:11px;color:rgba(255,255,255,0.55);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:12px;">📊 Movimiento de Caja</div>
    <div class="caja-g">
      <div><div class="caja-l">Inicio</div><div class="caja-v">S/${POS.cajaInicial.toFixed(2)}</div></div>
      <div><div class="caja-l">+ Ingresos</div><div class="caja-v" style="color:#48BB78;">S/${POS.totalVentasHoy.toFixed(2)}</div></div>
      <div><div class="caja-l">- Gastos</div><div class="caja-v" style="color:#FC8181;">S/${POS.totalGastosHoy.toFixed(2)}</div></div>
      <div><div class="caja-l">= En caja</div><div class="caja-v" style="color:#F6AD55;">S/${totalCaja.toFixed(2)}</div></div>
    </div>
  </div>

  ${seccionArqueo}

  <div class="sec-t">📋 Detalle de Ventas (${POS.ventasHoy.length})</div>
  <table>
    <thead>
      <tr>
        <th>Hora</th><th>Producto</th><th style="text-align:center;">Cant</th>
        <th style="text-align:right;">Precio</th><th style="text-align:right;">Costo</th>
        <th style="text-align:right;">Ganancia</th><th>Pago</th><th>Cliente</th>
      </tr>
    </thead>
    <tbody>
      ${filasVentas || '<tr><td colspan="8" style="text-align:center;padding:20px;color:#A0AEC0;">Sin ventas</td></tr>'}
    </tbody>
    ${POS.ventasHoy.length > 0 ? `
    <tfoot>
      <tr style="background:#EBF4FF;">
        <td colspan="3" style="padding:10px;font-weight:800;">TOTALES</td>
        <td style="padding:10px;text-align:right;font-weight:900;color:#2E6DB4;">S/${POS.totalVentasHoy.toFixed(2)}</td>
        <td style="padding:10px;text-align:right;color:#718096;">S/${POS.ventasHoy.reduce((s,v)=>s+v.costo,0).toFixed(2)}</td>
        <td style="padding:10px;text-align:right;font-weight:900;color:${POS.totalGananciaHoy>=0?'#276749':'#C53030'};">S/${POS.totalGananciaHoy.toFixed(2)}</td>
        <td colspan="2"></td>
      </tr>
    </tfoot>` : ''}
  </table>

  <div class="sec-t">💳 Desglose por Método de Pago</div>
  <table>
    <thead><tr><th>Método</th><th style="text-align:right;">Monto</th><th style="text-align:right;">%</th></tr></thead>
    <tbody>${filasPago || '<tr><td colspan="3" style="text-align:center;padding:14px;color:#A0AEC0;">Sin datos</td></tr>'}</tbody>
  </table>

  <div class="footer">
    <p style="font-weight:700;color:#4A5568;font-size:12px;margin-bottom:4px;">CellCom Tecnology — Sistema POS v1.0.0</p>
    <p>Generado el ${fecha} a las ${hora} · Cieneguilla, Lima, Perú · Operador: ${POS.usuario?.nombre || '—'}</p>
  </div>
  ...
  if (ventana) {
    ...
    ventana.focus();
    mostrarToast('📄 PDF generado — Ctrl+P para guardar', 'success');
  } else {
...
function configurarAtajosTeclado() {
  document.addEventListener('keydown', e => {
    // No interceptar si está en campos de texto
    const activoId = document.activeElement?.id;
    if (activoId === 'inputProducto' || activoId === 'inputDNI') return;
    ...
    if (e.key === 'Tab') {
      ...
      activarCampo(campos[(idx + 1) % campos.length]);
    }

    if (e.key === 'F2') { e.preventDefault(); document.getElementById('inputProducto')?.focus(); }
    if (e.key === 'F3') { e.preventDefault(); document.getElementById('inputDNI')?.focus(); }
    if (e.key === 'F4') { e.preventDefault(); seleccionarSeProbo('SI'); }
    if (e.key === 'F5') { e.preventDefault(); seleccionarSeProbo('NO'); }
  });
}
...
function mostrarToast(mensaje, tipo = 'info', duracion = 3000) {
  const colores = {
    success: { bg: 'linear-gradient(135deg, #276749, #48BB78)', dur: duracion },
    error:   { bg: 'linear-gradient(135deg, #C53030, #FC8181)', dur: duracion || 4000 },
    warning: { bg: 'linear-gradient(135deg, #C05621, #F6AD55)', dur: duracion || 3500 },
    info:    { bg: 'linear-gradient(135deg, #1A4F8A, #4A90D9)', dur: duracion || 2500 }
  };
